...
contract('SAEToken', function (accounts) {
	...
	it('initializes the contract with the correct values', function () {
		return SAEToken.deployed().then(function (instance) {
			...
			assert.equal(name, 'SAE Token', 'has the correct name');
			return tokenInstance.symbol();
		}).then(function (symbol) {
			assert.equal(symbol, 'SAE', 'has the correct symbol');
			return tokenInstance.standard();
		}).then(function (standard) {
			assert.equal(standard, 'SAE Token v1.0', 'has the  correct standard');
		});
	});
	...
	it('handles delegated token transfers', function () {
		return SAEToken.deployed().then(function (instance) {
			...
			//Approve spendingAccount to spend 10 tokens form fromAccounts
			...
			// Try transferring something larger than the appoved amount
